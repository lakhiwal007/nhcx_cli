...
import { api } from "../../api";
import { Card, Button, DocumentChecklist } from "../Common";

export default function PreauthDraft({ ctx }) {
  const navigate = useNavigate();
  const { cashlessCase, caseState, updateCaseState } = ctx;
  ...
  if (loading) {
    return (
      <div className="flex-center py-20 flex-col">
        ...
        <p className="text-muted">Generating Preauth Draft...</p>
      </div>
    );
  }
  ...
  const handleUpload = (docToUpload) => {
    ...
    const updatedDocs = draft.supporting_documents.map((d) =>
      d.code === docToUpload.code
        ? { ...d, url: "https://mock-url.com/doc.pdf" }
        : d,
    );
  ...
  const hasMissingDocs = draft?.supporting_documents?.some(
    (d) => !d.optional && !d.url,
  );
                ...
                <table className="table-modern" style={{ fontSize: "13px" }}>
                  ...
                  <tbody>
                    {draft?.items?.map((item, i) => (
                      <tr key={i}>
                        <td>
                          {item.service_name} <code>({item.service_code})</code>
                        </td>
                        <td>{item.quantity}</td>
